 ...
 * @returns 空化状态
 */
export function determineCavitationState(xF, xFz, FL) {
    const FL2 = FL * FL;
    if (xF <= xFz) {
        return '无空化';
    }
 ...
 * @returns 空化起始压差比
 */
function calculateXFz(Cv, Fd, FL) {
 ...
 * @returns 有效压差 KPa
 */
function calculateEffectiveDeltaP(deltaP, FL, P1, Pv) {
 ...
 * @returns 缩流断面流速 m/s
 */
function calculateVenaContractaVelocity(FL, deltaPc, rhoL) {
    // deltaPc从KPa转换为Pa
 ...
 * @returns 紊流声效系数
 */
function calculateTurbulentEfficiency(Uvc, cL) {
 ...
 * @returns 空化声效系数
 */
function calculateCavitationEfficiency(etaTurb, xF, xFzp, P1, P2, deltaPc) {
    // 确保参数有效
 ...
 * @returns 机械功率 W
 */
function calculateMechanicalPower(massFlow, Uvc, FL) {
 ...
 * @returns 声功率比
 */
function getSoundPowerRatio(valveType = 'standard') {
    switch (valveType) {
        case 'cage':
 ...
 * @returns Strouhal数
 */
function calculateStrouhalNumber(FL, Kv, Fd, xFzp, d, d0, P1, Pv) {
 ...
 * @returns 峰值频率 Hz
 */
function calculatePeakFrequencyTurbulent(Nstr, Uvc, Dj) {
    if (Dj <= 0)
        return 1000; // 默认值
    return Nstr * Uvc / Dj;
}
 ...
 * @returns 峰值频率 Hz
 */
function calculatePeakFrequencyCavitation(fpTurb, xF, xFzp) {
    if (xF <= 0 || xFzp <= 0 || xF >= 1)
        return fpTurb;
 ...
 * @returns 内部声压级 dB
 */
function calculateInternalNoiseLevel(Wa, rhoL, cL, Di) {
    if (Wa <= 0)
 ...
 * @returns 透射损失 dB
 */
function calculateTransmissionLoss(tp, fp, Di, pipeMaterial = 'steel') {
 ...
 * @returns 空化透射损失 dB
 */
function calculateCavitationTransmissionLoss(TL_turb, fpTurb, fpCav, etaTurb, etaCav) {
    if (fpTurb <= 0 || etaTurb + etaCav <= 0)
        return TL_turb;
 ...
 * @returns 外部噪音级 dB
 */
function calculateExternalNoiseLevel(Lpi, TL) {
    // +3 dB 为距离修正 (距阀门1m处)
    return Lpi - TL + 3;
}
/**
 ...
 * @returns 噪音计算结果
 */
export function calculateLiquidNoise(input) {
    const warnings = [];
    // 提取参数
    const { P1, P2, deltaP, massFlow, volumeFlow, density: rhoL, Pv = 2.34, // 默认20℃水的饱和蒸汽压 KPa
    soundSpeed: cL = NOISE_CONSTANTS.WATER_SOUND_SPEED, Kv, Cv, FL, Fd = 0.42, Di, tp, d, pipeMaterial = 'steel' } = input;
    // 计算质量流量 (如果只提供体积流量)
 ...
 * 获取空化状态描述
 */
export function getCavitationStateDescription(state) {
    const descriptions = {
        '无空化': '压差比低于空化起始点，流动平稳',
        '初生空化': '开始出现空化气泡，噪音增加',
        '恒定空化': '稳定空化状态，噪音和振动明显',
