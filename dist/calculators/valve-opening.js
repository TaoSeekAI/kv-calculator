 ...
 * @param R 固有可调比
 */
export function calcEqualPercentageOpening(m, R) {
    if (m <= 0 || R <= 1)
        return NaN;
 ...
 * @param R 固有可调比
 */
export function calcLinearOpening(m, R) {
    if (m <= 0 || R <= 1)
        return NaN;
 ...
 * @param R 固有可调比
 */
export function calcQuickOpeningOpening(m, R) {
    if (m <= 1 || R <= 1)
        return NaN;
 ...
 * @param flowChar 流量特性
 */
export function calcValveOpening(calculatedKv, ratedKv, rangeability, flowChar) {
