 ...
 * 基于 IEC 60534-8-3 标准
 */
import { NOISE_CONSTANTS, calculateAWeighting } from './noise/constants.js';
/**
 * 计算入口声速
 ...
 * @returns 入口声速 m/s
 */
function calculateInletSoundSpeed(gamma, T1, M) {
    // R = 8314.46 J/(kmol·K)
 ...
 */
function calculateOutletSoundSpeed(c1) {
 ...
 */
function calculateOutletDensity(rho1, P2, P1) {
    return rho1 * P2 / P1;
}
/**
 * 判定气体流动状态
 ...
 * @returns 流动状态和关键压力值
 */
export function determineGasFlowState(P1, P2, xT, gamma, FL = 0.9) {
    // 转换为Pa进行计算 (Excel使用Pa)
    const P1_Pa = P1 * 1000;
    const P2_Pa = P2 * 1000;
    // 临界缩流断面压力 Pvcc (E40)
 ...
 * @returns 缩流断面压力 KPa
 */
function calculateVenaContractaPressure(P1, P2, FL, state, Pvcc) {
    if (state === 'State I') {
 ...
 * @returns 马赫数
 */
function calculateVenaContractaMach(P1, Pvc, gamma, state) {
    if (state === 'State I') {
 ...
 */
function calculateVenaContractaVelocity(Mvc, c1, P1, Pvc, gamma) {
 ...
 * @param gamma 比热比
 */
function calculateJetMachNumber(P1, P2, alpha, gamma) {
 ...
 * @param gamma 比热比
 */
function calculateJetMachNumberStateV(gamma) {
 ...
 * @returns 声效系数
 */
function calculateAcousticEfficiency(state, Mvc, Mj, FL) {
    // 确保马赫数有效
 ...
 * @returns 峰值频率 Hz
 */
function calculatePeakFrequency(Uvc, d) {
    // 转换mm到m
    const d_m = d / 1000;
 ...
 * @returns 出口流速 m/s
 */
function calculateOutletVelocity(massFlow, rho2, Di) {
    // 转换单位
 ...
 * @returns 机械功率 W
 */
function calculateMechanicalPower(massFlow, Uvc) {
    const m_s = massFlow / 3600; // kg/h -> kg/s
 ...
 * Wa = η * Wm
 */
function calculateSoundPower(eta, Wm) {
    return eta * Wm;
}
/**
 * 计算内部声压级
 ...
 * @returns 内部声压级 dB
 */
function calculateInternalNoiseLevel(Wa, rho2, c2, Di) {
    if (Wa <= 0)
        return NOISE_CONSTANTS.MIN_NOISE;
 ...
 * @returns 透射损失 dB
 */
function calculateTransmissionLoss(tp, fp, Di, rho2, c2, pipeMaterial = 'steel') {
    // 转换单位 (输入为mm，需要转为m)
    const tp_m = tp / 1000;
    const Di_m = Di / 1000;
    // 环频率 fr (E90)
 ...
 * @returns 外部噪音级 dB
 */
function calculateExternalNoiseLevel(Lpi, TL, Di, tp) {
 ...
 * @returns 噪音计算结果
 */
export function calculateGasNoise(input) {
    const warnings = [];
    // 提取参数
    const { P1, P2, deltaP, T1, massFlow, density: rho1, gamma = 1.4, molecularWeight: M = 29, FL = 0.9, xT = 0.72, Di, tp, d, pipeMaterial = 'steel' } = input;
    // 计算比热比系数
 ...
 */
export function getGasFlowStateDescription(state) {
    const descriptions = {
