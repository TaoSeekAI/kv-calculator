...
export interface ActuatorSpec {
  model: string;           // 型号
  airConnection: string;   // 气源接口
  torque04: number | null; // 0.4MPa扭矩 Nm
  torque05: number | null; // 0.5MPa扭矩 Nm
  handwheel?: string;      // 手轮型号
}

export interface ActuatorType {
  type: string;            // 类型名称
  singleAction: ActuatorSpec[];  // 单作用
  doubleAction: ActuatorSpec[];  // 双作用
}
...
// 华尔式 拨叉气缸数据
const huaerShiBoChaData: ActuatorType = {
  type: '拨叉气缸',
  singleAction: [
...
// 永盛 齿轮齿条气缸数据
const yongShengChiLunData: ActuatorType = {
  type: '齿轮齿条气缸',
  singleAction: [
...
// 执行机构品牌数据
export const actuatorBrandData: ActuatorBrand[] = [
  {
    brand: '华尔式',
 ...
 * 获取所有品牌列表
 */
export function getActuatorBrands(): string[] {
 ...
 * 根据品牌获取类型列表
 */
export function getActuatorTypes(brand: string): string[] {
 ...
 */
export function getActuatorModels(brand: string, type: string, actionMode: 'single' | 'double' | 'all' = 'all'): string[] {
  const brandData = actuatorBrandData.find(b => b.brand === brand);
  if (!brandData) return [];
 ...
 * 根据型号获取执行机构规格
 */
export function getActuatorSpec(brand: string, type: string, model: string): ActuatorSpec | null {
  const brandData = actuatorBrandData.find(b => b.brand === brand);
 ...
 * 判断型号是单作用还是双作用
 */
export function getActionMode(model: string): '单作用' | '双作用' | null {
  if (model.includes('SR') || model.includes('sr')) return '单作用';
  if (model.includes('DA') || model.includes('da')) return '双作用';
  return null;
}
