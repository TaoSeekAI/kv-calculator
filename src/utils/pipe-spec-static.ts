 ...
 * 格式: 'DN-SCH': PipeSpec
 */
export const PIPE_SPECS_DATA: Record<string, PipeSpec> = {
  // DN=6 (1/8")
 ...
 * 内径 = 外径 - 2 × 壁厚
 */
export const PIPE_SPECS_METRIC: Record<string, PipeSpec> = {
  // DN=10 (公制外径 14mm)
...
export function getPipeSpec(
  dn: number,
  schStandard: string = '40',
  pipeStandard: PipeStandard = 'metric'
): PipeSpec | null {
 ...
 * @returns SCH标准列表
 */
export function getAvailableSchStandards(dn: number, pipeStandard: PipeStandard = 'metric'): string[] {
  ...
  return standards.sort((a, b) => {
    // 标准排序: STD, XS, XXS, 数字
    const order: Record<string, number> = { 'STD': 1, 'XS': 2, 'XXS': 3 };
 ...
 * @returns DN列表
 */
export function getAllSupportedDN(pipeStandard: PipeStandard = 'metric'): number[] {
  const dnSet = new Set<number>();
  const data = pipeStandard === 'metric' ? PIPE_SPECS_METRIC : PIPE_SPECS_DATA;
